...
# Install MySQL server in parallel on all HA nodes
#
# Usage: ./install-mysql.sh [node_list]
# Example: ./install-mysql.sh orange1,orange2,orange3
#          ./install-mysql.sh node1,node2
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}MySQL Parallel Installation Script${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
...
echo -e "${GREEN}========================================${NC}"
echo ""
echo "You can check MySQL service status with the following commands:"
for node in "${NODES[@]}"; do
