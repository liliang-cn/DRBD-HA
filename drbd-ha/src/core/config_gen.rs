...
impl ConfigGenerator {
    ...
    pub fn new() -> AppResult<Self> {
        ...
        tera.add_raw_template("drbd_resource.res", DRBD_RESOURCE_TEMPLATE)
            .map_err(|e| AppError::Config(format!("Failed to load DRBD template: {}", e)))?;

        // Add drbd-reactor promoter template
        tera.add_raw_template("promoter.toml", PROMOTER_TEMPLATE)
            .map_err(|e| AppError::Config(format!("Failed to load promoter template: {}", e)))?;
    ...
    pub fn generate_drbd_resource(&self, config: &ResourceConfig) -> AppResult<String> {
        ...
        self.tera
            ...
            .map_err(|e| AppError::Config(format!("Failed to render DRBD resource: {}", e)))
    }
    ...
    pub fn generate_promoter(&self, config: &PromoterPluginConfig) -> AppResult<String> {
        ...
        self.tera
            ...
            .map_err(|e| AppError::Config(format!("Failed to render promoter: {}", e)))
    }
    ...
    pub fn resource_from_request(
        ...
        // Generate a random device number (1000-9999) to avoid conflicts with minor numbers
        let device_num = (req.minor % 9000) + 1000; // Map minor (0-99999) to device (1000-9999)
