...
impl MetricsCollector {
    ...
    fn with_registry_internal(registry: Arc<Registry>) -> Result<Self> {
        ...
        let nodes_total = IntGauge::with_opts(
            Opts::new("drbd_ha_nodes_total", "Total number of nodes in cluster")
        )?;
        registry.register(Box::new(nodes_total.clone()))?;
        
        let nodes_online = IntGauge::with_opts(
            Opts::new("drbd_ha_nodes_online", "Number of online nodes")
        )?;
        registry.register(Box::new(nodes_online.clone()))?;
        
        let resources_total = IntGauge::with_opts(
            Opts::new("drbd_ha_resources_total", "Total number of DRBD resources")
        )?;
        registry.register(Box::new(resources_total.clone()))?;
        
        let resources_healthy = IntGauge::with_opts(
            Opts::new("drbd_ha_resources_healthy", "Number of healthy DRBD resources")
        )?;
        registry.register(Box::new(resources_healthy.clone()))?;
        
        let resources_degraded = IntGauge::with_opts(
            Opts::new("drbd_ha_resources_degraded", "Number of degraded DRBD resources")
        )?;
        registry.register(Box::new(resources_degraded.clone()))?;
        
        // DRBD specific metrics
        let drbd_connections = IntGauge::with_opts(
            Opts::new("drbd_ha_drbd_connections", "Number of DRBD connections")
        )?;
        registry.register(Box::new(drbd_connections.clone()))?;
        
        let drbd_sync_bytes_total = IntCounterVec::new(
            Opts::new("drbd_ha_drbd_sync_bytes_total", "Total bytes synced by DRBD"),
            &["resource"]
        )?;
        ...
        let drbd_split_brain_events = IntCounter::with_opts(
            Opts::new("drbd_ha_drbd_split_brain_events_total", "Total number of split-brain events")
        )?;
        registry.register(Box::new(drbd_split_brain_events.clone()))?;
        
        // HA profiles metrics
        let ha_profiles_total = IntGauge::with_opts(
            Opts::new("drbd_ha_profiles_total", "Total number of HA profiles")
        )?;
        ...
        // Storage metrics
        let storage_pools_total = IntGauge::with_opts(
            Opts::new("drbd_ha_storage_pools_total", "Total number of storage pools")
        )?;
        registry.register(Box::new(storage_pools_total.clone()))?;
        
        let storage_volumes_total = IntGauge::with_opts(
            Opts::new("drbd_ha_storage_volumes_total", "Total number of storage volumes")
        )?;
        ...
        let api_request_duration = HistogramVec::new(
            HistogramOpts::new("drbd_ha_api_request_duration_seconds", "API request duration in seconds"),
            &["method", "endpoint"]
        )?;
        registry.register(Box::new(api_request_duration.clone()))?;
        
        let api_active_connections = IntGauge::with_opts(
            Opts::new("drbd_ha_api_active_connections", "Number of active API connections")
        )?;
        ...
        let operation_duration = HistogramVec::new(
            HistogramOpts::new("drbd_ha_operation_duration_seconds", "Operation duration in seconds"),
            &["operation_type"]
        )?;
        registry.register(Box::new(operation_duration.clone()))?;
        
        let operation_failures = IntCounterVec::new(
            Opts::new("drbd_ha_operation_failures_total", "Total number of operation failures"),
            &["operation_type", "error_type"]
        )?;
        registry.register(Box::new(operation_failures.clone()))?;
        
        // Data migration metrics
        let migration_operations_total = IntCounter::with_opts(
            Opts::new("drbd_ha_migration_operations_total", "Total number of migration operations")
        )?;
        registry.register(Box::new(migration_operations_total.clone()))?;
        
        let migration_bytes_transferred = IntCounter::with_opts(
            Opts::new("drbd_ha_migration_bytes_transferred_total", "Total bytes transferred during migrations")
        )?;
        registry.register(Box::new(migration_bytes_transferred.clone()))?;
        
        let migration_duration = Histogram::with_opts(
            HistogramOpts::new("drbd_ha_migration_duration_seconds", "Migration duration in seconds")
        )?;
    ...
    pub fn update_system_metrics(&self, nodes: usize, online: usize, resources: usize, healthy: usize, degraded: usize) {
    ...
    pub fn update_drbd_sync_progress(&self, resource: &str, progress: f64) {
        self.drbd_sync_progress.with_label_values(&[resource]).set(progress);
    }
    
    /// Increment DRBD sync bytes
    pub fn inc_drbd_sync_bytes(&self, resource: &str, bytes: u64) {
        self.drbd_sync_bytes_total.with_label_values(&[resource]).inc_by(bytes as u64);
    }
    ...
    pub fn update_ha_profiles_metrics(&self, total: usize, active_by_type: &[(&str, usize)], standby_by_type: &[(&str, usize)], failed_by_type: &[(&str, usize)]) {
        self.ha_profiles_total.set(total as i64);
        
        for (profile_type, count) in active_by_type {
            self.ha_profiles_active.with_label_values(&[profile_type]).set(*count as i64);
        }
        
        for (profile_type, count) in standby_by_type {
            self.ha_profiles_standby.with_label_values(&[profile_type]).set(*count as i64);
        }
        
        for (profile_type, count) in failed_by_type {
            self.ha_profiles_failed.with_label_values(&[profile_type]).set(*count as i64);
        }
    }
    
    /// Update storage metrics
    pub fn update_storage_metrics(&self, pools: usize, volumes: usize, pool_usage: &[(&str, u64, u64)]) {
        ...
        for (pool_name, used, free) in pool_usage {
            self.storage_used_bytes.with_label_values(&[pool_name]).set(*used as f64);
            self.storage_free_bytes.with_label_values(&[pool_name]).set(*free as f64);
        }
    }
...
pub static METRICS: Lazy<MetricsCollector> = Lazy::new(|| {
