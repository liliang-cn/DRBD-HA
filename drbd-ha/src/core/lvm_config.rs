...
const LVM_DRBD_FILTER: &str = r###"    filter = [ \"a|/dev/sd.*|\", \"a|/dev/nvme.*|\", \"r|/dev/drbd.*|\", \"r|.*|\" ]"###; // Using raw string for regex
...
pub async fn configure_lvm_filter() -> Result<()> {
    ...
    if filter_regex.is_match(&modified_content) {
        modified_content = filter_regex.replace_all(&modified_content, drbd_filter_line).to_string();
        filter_found = true;
    ...
    if global_filter_regex.is_match(&modified_content) {
        ...
        if !filter_found {
            modified_content = global_filter_regex.replace_all(&modified_content, drbd_filter_line).to_string();
            filter_found = true;
            info!("Updated existing 'global_filter' setting in {}", LVM_CONF_PATH);
        } else {
    ...
    if !filter_found {
        ...
        if let Some(captures) = devices_start_regex.captures(&modified_content) {
            if let Some(m) = captures.get(0) {
                ...
                info!("Added 'filter' setting to 'devices' section in {}", LVM_CONF_PATH);
            }
        } else {
            bail!("Could not find 'devices {{' section in {}. Please configure LVM filter manually.", LVM_CONF_PATH);
        }
    }
    ...
    if modified_content != content {
        fs::write(LVM_CONF_PATH, modified_content.as_bytes())
            .await
            .context(format!("Failed to write to {}", LVM_CONF_PATH))?;
        info!("Successfully configured LVM filter in {}", LVM_CONF_PATH);
    } else {
        info!("LVM filter already correctly configured in {}", LVM_CONF_PATH);
    }
