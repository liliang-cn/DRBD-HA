...
pub async fn get_metrics(State(_state): State<Arc<AppState>>) -> Result<Response, AppError> {
    ...
    match METRICS.export() {
        ...
        Err(e) => {
            ...
            Err(AppError::Internal(format!("Failed to export metrics: {}", e)))
        }
    }
}
...
pub async fn get_metrics_summary(State(_state): State<Arc<AppState>>) -> Result<impl IntoResponse, AppError> {
...
pub async fn health_with_metrics(State(_state): State<Arc<AppState>>) -> Result<impl IntoResponse, AppError> {
...
mod tests {
    ...
    async fn test_get_metrics_summary() {
        let state = create_test_state();
        let app = axum::Router::new()
            .route("/api/v1/metrics/summary", axum::routing::get(get_metrics_summary))
            .with_state(state);
    ...
    async fn test_health_with_metrics() {
        let state = create_test_state();
        let app = axum::Router::new()
            .route("/api/v1/health/metrics", axum::routing::get(health_with_metrics))
            .with_state(state);
